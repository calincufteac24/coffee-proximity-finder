...
| Class | LinkedIn Bio |
|---|---|
| `CoffeeShops::Finder` | *"I find the closest coffee shops. That's it. That's the whole bio."* |
| `CoffeeShops::Synchronizer` | *"I sync CSV data to the database in batches. I don't validate. I delegate."* |
| `Csv::Fetcher` | *"I fetch URLs. I don't parse. I don't judge."* |
| `Csv::Parser` | *"I turn messy CSV lines into clean hashes. The rows I reject don't get a goodbye."* |
| `CoordinateValidator` | *"Is 47.6 a valid latitude? Yes. Is 999? No. Meeting adjourned."* |
| `CsvRowValidator` | *"Three columns? ✅ Script injection? 🚫 Empty name? 🚫 I have trust issues."* |
| `CoffeeShopSerializer` | *"I make JSON look pretty. JSON:API spec. No debates."* |
...
Fully compliant with the [JSON:API Specification](https://jsonapi.org/). Yes, we read the spec. All of it. No, we don't want to talk about it. I promise.
...
|---|---|---|---|
| **Validators** | `coordinate_validator_test.rb` | 22 | Boundaries ±90/±180, letters, nil, empty, special chars |
| **Validators** | `csv_row_validator_test.rb` | 11 | Column count, XSS injection, unicode, 255-char limit |
| **Services** | `csv/parser_test.rb` | 14 | Nil content, malformed rows, quoted commas, logging |
| **Services** | `csv/fetcher_test.rb` | 3 | HTTP 200/500/404 with dependency injection |
| **Services** | `coffee_shops/finder_test.rb` | 12 | Sort order, zero distance, limit, custom scope, empty DB |
| **Services** | `coffee_shops/synchronizer_test.rb` | 9 | Idempotent upsert, fetch errors, timestamps, batches |
| **Integration** | `coffee_shops_controller_test.rb` | 20 | JSON:API structure, validation, SQL injection attempts |
| **Model** | `coffee_shop_test.rb` | 18 | Validations, external_id generation |
...
Because even APIs deserve a nice outfit sometimes.
...
|---|---|---|
| **Framework** | Rails 8.1 | Convention over configuration. Uncle Bob would debate this, but we like it. |
| **Database** | PostgreSQL | Does the distance math in C, not Ruby. 1000x faster. Literally. |
| **Background Jobs** | Solid Queue | Uses the existing PostgreSQL. No Redis tax. |
| **Serialization** | jsonapi-serializer | JSON:API spec without the headache. |
| **CSS** | Tailwind 4 | Utility-first. Like SOLID, but for stylesheets. |
| **JS** | Stimulus + Turbo | Just enough JavaScript. Not a single `node_modules` black hole. |
...
| `SOLID_QUEUE_IN_PUMA` | `true` — runs background jobs inside Puma |
